...
use std::io::{self, stdout, Write, Read, Error, ErrorKind};

use curl::easy::{Easy, List};
use duct;
use std::ffi::OsString;

pub fn get_wan_name() -> io::Result<String> {

    let output = duct::cmd(OsString::from("/bin/ip"),
                           vec!(OsString::from("route")))
        .read()
        .map_err(|e|io::Error::new(io::ErrorKind::NotFound, "/bin/ip not found."))?;
    ...
    Err(Error::new(ErrorKind::NotFound, "Not Found wan dev name"))
}
            ...
            let socket1 = match TcpStream::connect_timeout(&addr1, timeout) {
                ...
                Err(e) => {
                    #[cfg(unix)]
                    {
                        ...
                        let output = duct::cmd(OsString::from("/bin/ip"),
                                               vec!(OsString::from("address")))
                            .read()
                            .unwrap();
                        let ip_str: &str = output
                            .lines()
                            .find(|line| {
                                if let Some(_) = line.trim().find(&dev) {
                                    if let Some(_) = line.trim().find("inet") {
                                        return true;
                                    }
                                }
                                return false;
                        ...
                        let ip: Vec<&str> = ip_str.split("/").collect();
